...
把“帮我写点东西”升级成“能稳定产出、能评测、能复用、能防注入”的中文 Prompt 工程方案。
...
- 中文 Prompt 六段式：角色设定、任务目标、人格风格、行为约束、示例引导、输出契约。
- 支持中国模型适配：DeepSeek、Qwen、GLM、Kimi、豆包、MiniMax、百度文心。
- 覆盖真实中文场景：简历优化、小红书、公众号、电商、编程、教育、会议纪要。
- 内置 Prompt 优化器：把模糊需求拆成缺失信息、优化 Prompt 和可执行任务。
- 中文 Prompt Injection 防护：覆盖中文直攻、角色扮演、RAG 间接注入。
- 本地化测试集：用于比较不同模型在中文任务上的稳定性。

## 一眼看懂

### 例子 1：小红书不是“帮我写一篇”就完了
...
- 能承接网络热点，但不要贩卖焦虑。
- 每个选题都要有明确观点，而不是只堆情绪。
- 避免“所有人”“必须”“毁掉一生”等绝对化表达。

输出表格：
| 选题标题 | 核心观点 | 适合平台 | 情绪强度 | 建设性落点 |
```

### 例子 4：中文 Prompt Injection 也要专门防
